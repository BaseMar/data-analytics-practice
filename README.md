# Data Analytics Practice

## About
This repository documents my journey of becoming a Data Analyst.

I focus on building practical skills in:
- SQL (data querying, aggregation, joins)
- Python (data manipulation and problem solving)
- Analytical thinking on real datasets

---

## What you will find here
- SQL queries solving analytical problems
- Python exercises focused on working with data
- Step-by-step improvement over time

---

## Learning approach
I practice daily using platforms like:
- HackerRank: [https://www.hackerrank.com/](https://www.hackerrank.com/profile/BaseMar)
- Codewars: [https://www.codewars.com/](https://www.codewars.com/users/BaseMar)

The focus is on building practical data analysis skills:
- working with structured data (SQL)
- processing data in Python
- understanding real-world data problems
---

## Current focus
- SQL: aggregations, filtering, preparing for joins
- Python: data structures and basic data manipulation

---

## Progress
- 22.04.2026: Basic SQL queries (SELECT, WHERE, AVG, MIN/MAX)
- 24.04.2026: Python: Average calculation, Positive vs negative analysis, Data cleaning (extracting numbers)
- 24.04.2026: Basic data retrieval (SELECT), Filtering data (MOD, DISTINCT), Detecting duplicates (COUNT vs COUNT DISTINCT), String manipulation (CONCAT, LEFT), Aggregation (GROUP BY), Data formatting for reporting
---

## Goal
Become a job-ready Data Analyst by building strong fundamentals and working with real-world data.
